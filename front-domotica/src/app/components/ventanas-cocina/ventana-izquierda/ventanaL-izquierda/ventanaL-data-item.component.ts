...
@Component({
  selector: 'app-ventana-ii-data-item',
  standalone: true,
  imports: [
    MatCardModule,
    MatTooltipModule,
...
export class VentanaIIDataItemComponent {
  ...
  @Input('windowILeft-data') windowILeftData: iActuatorsData | null | undefined = undefined;
  ...
  @Output('actuator-updated') actuatorUpdated: EventEmitter<void> =
    ...
    ngOnChanges(changes: SimpleChanges): void {
      if (changes['windowILeftData'] && !changes['windowILeftData'].isFirstChange()){
        this.updateWindowRState();
      }
    }

    private updateWindowRState(): void {
      if (this.windowILeftData && this.windowILeftData.actions.length > 0) {
        const windowAction = this.windowILeftData.actions.find(a => a.name === 'Activación mecánica');
        if (windowAction){
  ...
  onToggleChange(event: any) {
    if(this.windowILeftData){
      const newState = event.target.checked ? 'Abierta' : 'Cerrada';
      const data = { value : newState };

      if(event.target.checked) {
        this.componentsService.doubleWindowsRightOpen(this.windowILeftData.arduinoIp).subscribe(
          (res) => {
            this.toggleState = true;
            this.windowsImage = 'assets/image/ventana_izq.jpg'
            this.actuatorUpdated.emit();
        ...
        this.componentsService.doubleWindowsRightClose(this.windowILeftData.arduinoIp).subscribe(
          (res) => {
            this.toggleState = false;
            this.windowsImage = 'assets/image/ventanaRL.jpg';
            this.actuatorUpdated.emit();
