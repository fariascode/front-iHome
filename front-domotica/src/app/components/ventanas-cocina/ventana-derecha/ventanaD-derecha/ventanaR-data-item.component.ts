...
@Component({
  selector: 'app-ventana-dd-data-item',
  standalone: true,
  imports: [
    MatCardModule,
    MatTooltipModule,
...
export class VentanaDDDataItemComponent {
  ...
  @Input('windowDRight-data') windowDRightData: iActuatorsData | null | undefined = undefined;
  ...
  @Output('actuator-updated') actuatorUpdated: EventEmitter<void> =
    ...
    ngOnChanges(changes: SimpleChanges): void {
      if (changes['windowDRightData'] && !changes['windowDRightData'].isFirstChange()){
        this.updateWindowRState();
      }
    }

    private updateWindowRState(): void {
      if (this.windowDRightData && this.windowDRightData.actions.length > 0) {
        const windowAction = this.windowDRightData.actions.find(a => a.name === 'Activación mecánica');
        if (windowAction){
  ...
  onToggleChange(event: any) {
    if(this.windowDRightData){
      const newState = event.target.checked ? 'Abierta' : 'Cerrada';
      const data = { value : newState };

      if(event.target.checked) {
        this.componentsService.doubleWindowsRightOpen(this.windowDRightData.arduinoIp).subscribe(
          (res) => {
            this.toggleState = true;
            this.windowsImage = 'assets/image/ventana_der.jpg'
            this.actuatorUpdated.emit();
        ...
        this.componentsService.doubleWindowsRightClose(this.windowDRightData.arduinoIp).subscribe(
          (res) => {
            this.toggleState = false;
            this.windowsImage = 'assets/image/ventanaRL.jpg';
            this.actuatorUpdated.emit();
